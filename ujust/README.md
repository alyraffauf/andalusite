...
Andalusite ships containerized services as [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html) templates. The convention has two halves:

1. **Catalog templates** in the OS image at `/usr/share/andalusite/quadlets/<name>.container`. These ship in `files/<variant>/usr/share/andalusite/quadlets/`, are owned by the image, and are never auto-loaded by systemd. They serve as immutable, OS-update-tracked source-of-truth templates.
...
Catalog template at `files/main/usr/share/andalusite/quadlets/example.container`:
...
enable-example:
    ...
    cp -n /usr/share/andalusite/quadlets/example.container \
...
2. If on a bootc system: `sudo bootc switch --target localhost/andalusite:stable`
