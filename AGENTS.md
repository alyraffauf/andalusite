# Agent Instructions for Andalusite

Andalusite is a bootc OS based on Fedora COSMIC Atomic (COSMIC desktop). Heavy/optional functionality lives in the sibling [kyanite-sysexts](https://github.com/alyraffauf/kyanite-sysexts) repo.
...
**Currently built:** `andalusite` (main only).
...
- `IMAGE_FLAVOR=main` → OS identifies as "Andalusite".

## OPTIONAL EXTENSIONS (sysexts)

Heavy/opt-in functionality is **not** baked into the andalusite image. It ships as systemd-sysext packages from [kyanite-sysexts](https://github.com/alyraffauf/kyanite-sysexts):
...
3. `03-third-party-packages.sh` - COPR packages (Ghostty, krunner-bazaar)
...
8. `08-branding.sh` - OS release branding (uses `SHA_HEAD_SHORT` build arg, invalidates per commit)
...
- [kyanite](https://github.com/alyraffauf/kyanite) - KDE Plasma sister image
