...
A Fedora COSMIC Atomic spin I run as my daily driver. COSMIC desktop, no corporate branding, rough edges sanded off.

Sister image to [Kyanite](https://github.com/alyraffauf/kyanite) (KDE Plasma), built the same way but based on Fedora's COSMIC Atomic image instead of Kinoite. Andalusite is a polymorph of kyanite — same chemistry, different crystal structure.

## What's different from stock COSMIC Atomic

- Firefox via Mozilla's official Flatpak; Bazaar in place of the COSMIC Store for Flatpak management.
- Flathub set up on first boot; Fedora's flatpak remotes removed.
- Fish as the default shell, fcitx5 for CJK input.
...
I borrow the [ujust](https://github.com/ublue-os/packages/tree/main/packages/ublue-os-just) command framework and a couple of utility packages from Universal Blue's COPR, but everything else is built straight on Fedora COSMIC Atomic.

## Quick Start

If you're on a bootc system already (COSMIC Atomic, Silverblue, etc.):

```bash
sudo bootc switch ghcr.io/alyraffauf/andalusite:stable
sudo systemctl reboot
```

After it boots, `ujust --list` shows what custom recipes are available. A couple things to know up front: stuff under `/etc/skel` doesn't auto-migrate, and rebasing across desktop environments (e.g. KDE → COSMIC) usually goes badly.
...
```bash
ujust enable-ollama          # CPU (or NVIDIA on a cosmic-atomic-nvidia base)
...
The variant scaffold (`packages.json variants.{name}`, `IMAGE_FLAVOR=NAME` in CI) is still wired up even though I only build `andalusite` now. Flipping a CI switch can revive `dx` or any other variant.
...
```bash
just build           # build the andalusite container
...
```bash
BASE_IMAGE=ghcr.io/ublue-os/cosmic-atomic-nvidia:latest \
BASE_IMAGE_SHA=$(skopeo inspect docker://$BASE_IMAGE --format '{{.Digest}}') \
just build
```

Output lands in `output/`. I don't publish pre-built ISOs — install Fedora COSMIC Atomic and rebase, or build one yourself.
...
```bash
cosign verify \
  --key https://raw.githubusercontent.com/alyraffauf/andalusite/main/cosign.pub \
  ghcr.io/alyraffauf/andalusite:stable
```

### Switching to signed transport

`ostree-image-signed:` only works once the running deployment ships andalusite's `policy.json` and `cosign.pub`. A fresh switch from non-andalusite is a two-step bootstrap:
...
sudo rpm-ostree rebase ostree-image-signed:docker://ghcr.io/alyraffauf/andalusite:stable
sudo systemctl reboot
```
...
It works well enough for me to use daily. This is intended to be a very light repackage of Fedora COSMIC Atomic.

## Resources

- [kyanite](https://github.com/alyraffauf/kyanite) — the KDE Plasma sister image
...
- [COSMIC](https://system76.com/cosmic/) — the desktop environment

## License

Apache 2.0. See [LICENSE.md](LICENSE.md). Based on [Fedora COSMIC Atomic](https://fedoraproject.org/atomic-desktops/cosmic/) with [COSMIC](https://system76.com/cosmic/), inspired by [Universal Blue](https://universal-blue.org/).
