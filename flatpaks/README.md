...
- `main.preinstall` - Base variant flatpaks (applies to all andalusite images)

Files are copied to `/usr/share/flatpak/preinstall.d/andalusite-{variant}.preinstall` in the built image based on the `IMAGE_FLAVOR` variable during the build.
